...
#### 10. ダウンロード機能
- **分析結果（Markdown形式）**: スコア、サマリ、詳細分析、改善案を含む完全なレポート
  - ファイル名: `analysis_result.md`
  - 形式: Markdown（読みやすい見出し構造）
  - 個人情報に関する注意書きを含む
- **応募メール下書き（テキスト形式）**: 件名案、本文、根拠リスト、注意事項
  - ファイル名: `email_draft.txt`
  - 形式: テキスト（コピー＆ペーストしやすい形式）
- **求人深掘りチャット履歴（Markdown形式）**: 会話履歴を保存
  - ファイル名: `job_chat_{mode}.md`
  - 形式: Markdown（会話ごとに見出し分け）
- **使い方**: 結果表示の下部にある「⬇️ ダウンロード」セクションから各ボタンをクリック
