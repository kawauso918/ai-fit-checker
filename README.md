...
#### 8-2. 応募メール下書き生成
- **宛名入力（任意）**: 
  - **会社名**: 会社名を入力すると、本文冒頭に「株式会社◯◯ 採用ご担当者様」のように反映されます
  - **担当者名**: 担当者名を入力すると、本文冒頭に「◯◯様」または「株式会社◯◯ ◯◯様」のように反映されます
  - **フォールバック**: どちらも未入力の場合は「採用ご担当者様」「貴社」を使用します
  - **注意**: 入力があるときだけ固有名詞として使用し、捏造で勝手に部署名等を追加しません
- **件名案（2〜3件）**: 複数の件名案から選択可能
- **本文テンプレート**: 丁寧で短め（300-500文字程度）の文面
  - 冒頭に宛名が自然に反映されます（入力された情報のみを使用）
...
- **使い方**: 分析結果表示後に「📝 宛名情報（任意）」を展開して会社名・担当者名を入力（任意）、その後「📝 応募メール下書きを生成」ボタンを押すと生成されます（コスト対策のため自動生成は行いません）
- **免責**: 生成された下書きは参考情報です。送信前に必ず内容を確認し、誤字脱字や企業名・役職名が正しいか確認してください

#### 9. 求人深掘りチャット
求人の解釈、応募メールの改善、面接質問の作成を目的別のモードでサポートします。

**モード切替機能**:
- **📖 求人理解モード**: 求人票の要件や業務内容を解釈し、応募者が理解すべきポイントを明確にします
- **📧 応募メール改善モード**: 応募メールの文面を改善し、より効果的な表現を提案します
- **❓ 面接質問作成モード**: 面接で想定される質問を作成し、回答の骨子を提示します

**各モードの出力フォーマット**:

1. **📖 求人理解モード**
   - **解釈**: 質問された要件や内容の具体的な解釈
   - **重要ポイント**: この要件が重要な理由、期待されるスキルや経験のレベル
   - **確認すべき点（質問案）**: 面接や応募前に確認すべき質問を3〜5個、箇条書きで提示
   - **次のアクション**: この要件を満たすために取るべき行動（学習、経験の整理など）

2. **📧 応募メール改善モード**
   - **改善方針**: 現在の文面の問題点と改善の方向性
   - **改善後の文案（短い）**: 改善後の文面を具体的に提示（100-200文字程度）
   - **修正理由**: なぜこの改善が効果的なのか、根拠を説明
   - **追加で必要な情報**: より良い文面を作るために確認すべき点や追加すべき情報

3. **❓ 面接質問作成モード**
   - **想定質問10個**: 面接で想定される質問を10個、番号付きで提示
   - **狙い**: 各質問の意図や評価ポイント
   - **回答の骨子（箇条書き）**: 各質問に対する回答の骨子を箇条書きで提示（職務経歴に基づく）
   - **逆質問案5個**: 面接官に質問すべき内容を5個、箇条書きで提示

**質問例**:

- **求人理解モード**:
  1. 「この要件『Python開発経験3年以上』は具体的に何を求めていますか？」
  2. 「『フルスタック開発経験』とはどのような業務を指しますか？」
  3. 「この求人で求められる技術レベルはどの程度ですか？」

- **応募メール改善モード**:
  1. 「応募メールの冒頭部分を改善したいです。現在の文面: [文面を貼り付け]」
  2. 「志望動機の書き方を教えてください」
  3. 「未経験の技術について、どのように表現すべきですか？」

- **面接質問作成モード**:
  1. 「この求人で想定される面接質問を作成してください」
  2. 「強みを深掘りする質問を作成してください」
  3. 「面接で質問すべき内容を教えてください」

**チャット履歴**: モードごとに履歴を保持し、継続的な対話が可能

#### 10. ダウンロード機能
分析結果、応募メール下書き、チャット履歴をファイル形式でダウンロードできます。

**ダウンロード可能なファイル**:

1. **分析結果（Markdown形式）**
   - ファイル名: `analysis_result.md`
   - 形式: Markdown（読みやすい見出し構造）
   - 内容: スコア、サマリ、詳細分析、改善案を含む完全なレポート
   - 個人情報に関する注意書きを含む
   - **注意**: 分析結果が存在する場合のみダウンロード可能

2. **応募メール下書き（テキスト形式）**
   - ファイル名: `email_draft.txt`
   - 形式: テキスト（コピー＆ペーストしやすい形式）
   - 内容: 件名案、本文、根拠リスト、注意事項、送信前確認事項
   - **注意**: 応募メール下書きを生成済みの場合のみダウンロード可能

3. **求人深掘りチャット履歴（Markdown形式）**
   - ファイル名: `job_chat_{mode}.md`（mode: job_understanding / email_improvement / interview_questions）
   - 形式: Markdown（会話ごとに見出し分け）
   - 内容: 現在選択中のモードのチャット履歴
   - **注意**: チャット履歴が存在する場合のみダウンロード可能

**使い方**:
- 結果表示の下部にある「⬇️ ダウンロード」セクションから各ボタンをクリック
- ボタンは、該当するデータが存在する場合のみ有効になります

**文字コード**: UTF-8でエンコード（文字化けしません）

**改行・引用の表示**:
- Markdown形式では、改行や引用が適切に表示されます
- テキスト形式では、改行が保持され、引用は引用符（>）で表示されます
...
1. **宛名情報を入力（任意）**
   ...
   - 会社名を入力（例: 株式会社◯◯）
   - 担当者名を入力（例: 田中太郎）
   - 未入力の場合は「採用ご担当者様」「貴社」を使用
   - **注意**: 入力があるときだけ固有名詞として使用し、捏造で勝手に部署名等を追加しません

2. **応募メール下書きを生成**
   - 「📝 応募メール下書きを生成」ボタンを押す
   - 件名案（2〜3件）から選択
   - 本文テンプレートをコピー（宛名が冒頭に反映されます）
...
5. **ダウンロード（任意）**
   - 「⬇️ ダウンロード」セクションから「📧 応募メール下書きをダウンロード (TXT)」をクリック
   - テキスト形式で保存され、後で確認・編集できます

#### ④ チャット深掘り（求人内容の理解を深める）
1. **モードを選択**
   - **📖 求人理解**: 求人票の要件や業務内容を解釈
   - **📧 応募メール改善**: 応募メールの文面を改善
   - **❓ 面接質問作成**: 面接で想定される質問を作成
   - **注意**: モードを切り替えると、そのモード専用の履歴が表示されます（履歴はモードごとに分離されます）

2. **チャット機能を使用**
   - 選択したモードに応じた質問を入力
   - **求人理解モード**: 「この要件『Python開発経験3年以上』は具体的に何を求めていますか？」
   - **応募メール改善モード**: 「応募メールの冒頭部分を改善したいです。現在の文面: [文面を貼り付け]」
   - **面接質問作成モード**: 「この求人で想定される面接質問を作成してください」

3. **モードごとの回答を確認**
   - 各モードに応じた構造化された回答が表示されます
   - 回答フォーマットはモードごとに異なります（詳細は「9. 求人深掘りチャット」セクションを参照）

4. **チャット履歴を確認**
   - モードごとに履歴が分離されて保存されます
   - 画面更新しても履歴は保持されます
   - モードを切り替えても、各モードの履歴は独立して保持されます

5. **ダウンロード（任意）**
   - 「⬇️ ダウンロード」セクションから「💬 チャット履歴をダウンロード (MD)」をクリック
   - 現在選択中のモードの履歴がMarkdown形式でダウンロードされます
