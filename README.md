...
- **入力バリデーション**:
  - APIキーの自動チェック（未設定時はエラー表示）
  - 文字数制限チェック（50,000文字上限、20,000文字以上で警告）

### 出力
- **Must要件の適合度**: 必須要件に対する適合状況
- **Want要件の適合度**: 歓迎要件に対する適合状況
- **根拠引用**: 評価の根拠となった具体的な記述の引用
  - **引用検証機能**: 職務経歴書からの引用が原文に存在するか自動検証し、存在しない場合は警告表示
- **スコア**: 総合的な適合度スコア（0-100点）
- **改善案**: 適合度を向上させるための具体的な提案（職務経歴書の編集案、行動計画）
...
1. **F1: 求人要件抽出** - 求人票からMust要件とWant要件を抽出
2. **F2: 根拠抽出** - 職務経歴書から各要件に対する根拠を抽出
3. **F3: スコア計算** - 適合度スコアを計算（Must/Want別）
4. **F4: 改善案生成** - ギャップ分析に基づいて改善案を生成
