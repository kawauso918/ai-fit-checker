...
- **企業情報（オプション）**: 会社概要、採用ページ全体などの情報を追加可能
  - 企業情報は「背景文脈」として補助的に利用されます
  - F1（要件抽出）では求人票の理解を深めるために使用
  - F4（改善提案）では志望動機や表現の寄せに活用
  - **注意**: 会社紹介・沿革・所在地などの情報は要件として抽出されません（カルチャーはWant要件として抽出可能）
